...
def db_stop(user_id: str):
    loging(logger_level='WARN', user_id=user_id, do='Admin will reboot the bot . . .')
    loging(logger_level='INFO', user_id=user_id, do='Saving data to db . . .')
    conn.commit()
    loging(logger_level='WARN', user_id=user_id, do='Disconnect from db . . .')
    conn.close()
    loging(logger_level='INFO', user_id=user_id, do='Successfully !')
    loging(logger_level='WARN', user_id=user_id, do='Rebooting . . .')